...
class TrainingEnvironment:
    ...
    Version avec points de mesure fixes
    """
    
    def __init__(self, fixed_distances=None):
        ...
        # Points de mesure fixes (les 13 distances)
        if fixed_distances is None:
            # À adapter selon vos 13 points de mesure réels
            self.fixed_distances = [
                0.1, 0.2, 0.5, 0.8, 1.0, 1.2, 1.5, 
                1.8, 2.0, 2.5, 3.0, 3.5, 4.0
            ]  # 13 points de 0.1 à 4.0 km
        else:
            self.fixed_distances = fixed_distances
        
        # Paramètres réalistes (adaptés au nouveau canal)
        self.max_distance_km = max(self.fixed_distances)
        self.min_distance_km = min(self.fixed_distances)
        self.max_payload_bytes = 230
        self.min_payload_bytes = 1
        
        # Paramètres canal
        self.noise_figure_db = 6.0
        self.path_loss_exponent = 3.3
        self.shadowing_std_db = 7.0
        self.reference_loss_db = 125.0
        self.seed_global = 42
        ...
        self.current_snr_history = deque(maxlen=3)
        self.success_history = deque(maxlen=5)
        self.retry_count = 0
        self.step_count = 0
        self.max_steps = 100
        self.current_payload_bytes = 50
        self.current_position = (0, 0)
        
        # Mode d'échantillonnage des distances
        self.distance_sampling_mode = 'random'  # 'sequential', 'random', 'curriculum'
        self.curriculum_phase = 0
        
        # Pour génération déterministe du shadowing
        self.shadowing_cache = {}
        
        logger.info(f"Environnement initialisé: {self.action_dim} actions")
        logger.info(f"Points de mesure fixes ({len(self.fixed_distances)}): {self.fixed_distances}")
    
    def get_state_dim(self):
        """Retourne la dimension de l'état"""
        return 8
    
    def get_action_dim(self):
        """Retourne la dimension de l'action"""
        return self.action_dim
    
    def set_sampling_mode(self, mode='sequential'):
        """Change le mode d'échantillonnage des distances"""
        valid_modes = ['sequential', 'random', 'curriculum']
        if mode not in valid_modes:
            raise ValueError(f"Mode doit être l'un de {valid_modes}")
        self.distance_sampling_mode = mode
        logger.info(f"Mode d'échantillonnage des distances: {mode}")
    ...
    def _select_distance(self):
        """Sélectionne une distance selon le mode choisi"""
        if self.distance_sampling_mode == 'sequential':
            # Parcourir séquentiellement les points
            self.current_distance_idx = (self.current_distance_idx + 1) % len(self.fixed_distances)
            return self.fixed_distances[self.current_distance_idx]
        
        elif self.distance_sampling_mode == 'random':
            # Échantillonnage aléatoire uniforme
            return np.random.choice(self.fixed_distances)
        
        elif self.distance_sampling_mode == 'curriculum':
            # Apprentissage curriculaire: commencer par les distances faciles
            total_phases = 4
            phase_size = len(self.fixed_distances) // total_phases
            
            # Déterminer la phase en fonction du step_count
            progress = min(1.0, self.step_count / (self.max_steps * 100))
            phase = min(total_phases - 1, int(progress * total_phases))
            
            # Sélectionner dans les distances de cette phase
            start_idx = 0
            end_idx = (phase + 1) * phase_size
            available_distances = self.fixed_distances[start_idx:end_idx]
            
            return np.random.choice(available_distances)
    
    def reset(self):
        """Réinitialise l'environnement pour un nouvel épisode"""
        # Sélectionner une distance selon le mode
        self.current_distance = self._select_distance()
        ...
        self.current_payload_bytes = np.random.randint(1, 231)
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        info = {
            ...
            'distance_idx': self.current_distance_idx
        }
        
        # Vérifier si l'épisode est terminé
        done = self.step_count >= self.max_steps
        
        # Pour le prochain état, on peut soit:
        # - Rester à la même distance (pas de mouvement)
        # - OU changer de point de mesure
        if not done and np.random.random() < 0.2:  # 20% de chance de changer
            self.current_distance = self._select_distance()
    ...
    def _calculate_reward(self, success: bool, tx_power: float, 
        ...
        toa_norm = min(toa_ms / 3813.376, 1.0)
...
def train_ddqn_fixed_points(
    num_episodes: int = 5000,
    save_dir: str = "./ddqn_checkpoints_fixed",
    resume_from: str = None,
    save_interval: int = 500,
    sampling_mode: str = 'random'  # 'sequential', 'random', 'curriculum'
):
    """
    Entraînement sur points de mesure fixes
    """
    os.makedirs(save_dir, exist_ok=True)
    
    # Définir les 13 points de mesure (à adapter selon vos données)
    fixed_distances = [
        0.1, 0.2, 0.5, 0.8, 1.0, 1.2, 1.5, 
        1.8, 2.0, 2.5, 3.0, 3.5, 4.0
    ]
    
    # Environnement avec points fixes
    env = TrainingEnvironment(fixed_distances=fixed_distances)
    env.set_sampling_mode(sampling_mode)
    ...
    logger.info("=" * 80)
    logger.info("ENTRAÎNEMENT DDQN SUR POINTS FIXES (13 DISTANCES)")
    logger.info(f"Points de mesure: {fixed_distances}")
    logger.info(f"Mode d'échantillonnage: {sampling_mode}")
    ...
    distance_stats = {d: {'rewards': [], 'success_rates': []} for d in fixed_distances}
    ...
    for episode in range(start_episode, num_episodes):
        ...
        # Enregistrer par distance
        if current_distance in distance_stats:
            distance_stats[current_distance]['rewards'].append(episode_reward)
            distance_stats[current_distance]['success_rates'].append(success_rate)
        ...
        if (episode + 1) % save_interval == 0:
            checkpoint_path = os.path.join(save_dir, f"ddqn_fixed_ep{episode+1}.pth")
            agent.save_model(checkpoint_path)
    
    # Sauvegarde finale
    final_path = os.path.join(save_dir, "ddqn_fixed_final.pth")
    agent.save_model(final_path)
    
    # Sauvegarde du modèle de déploiement
    deploy_path = os.path.join(save_dir, "ddqn_fixed_deploy.pth")
    torch.save(agent.policy_net.state_dict(), deploy_path)
    
    # Afficher les performances par distance
    logger.info("\n" + "=" * 80)
    logger.info("PERFORMANCES PAR DISTANCE:")
    logger.info("-" * 80)
    for dist in fixed_distances:
        if distance_stats[dist]['success_rates']:
            avg_success = np.mean(distance_stats[dist]['success_rates'][-50:])  # Derniers 50 épisodes
            avg_reward = np.mean(distance_stats[dist]['rewards'][-50:])
            logger.info(f"Distance {dist:.2f}km: Succès={avg_success:.1%}, Reward={avg_reward:.1f}")
...
def train_ddqn(
    ...
    export_stats_to_csv(agent.stats, save_dir)
...
def export_stats_to_csv(stats: Dict, save_dir: str):
    """Exporte les statistiques d'entraînement dans un fichier CSV"""
    import csv
    ...
    num_episodes = max(
        len(stats.get('episode_rewards', [])),
        len(stats.get('success_rates', [])),
        len(stats.get('losses', [])),
    )

    # Remplir les colonnes manquantes avec None si nécessaire
    def pad(lst, length):
        return list(lst) + [None] * (length - len(lst))

    rows = zip(
        range(1, num_episodes + 1),
        pad(stats.get('episode_rewards', []),   num_episodes),
        pad(stats.get('episode_lengths', []),   num_episodes),
        pad(stats.get('success_rates', []),     num_episodes),
        pad(stats.get('losses', []),            num_episodes),
        pad(stats.get('avg_snr', []),           num_episodes),
        pad(stats.get('avg_powers', []),        num_episodes),
        pad(stats.get('avg_drs', []),           num_episodes),
        pad(stats.get('avg_toa', []),           num_episodes),
        pad(stats.get('epsilon_values', []),    num_episodes),
    )
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for row in rows:
            writer.writerow([
                v if v is None else (round(v, 6) if isinstance(v, float) else v)
                for v in row
            ])

    logger.info(f"Données CSV sauvegardées: {csv_path} ({num_episodes} épisodes)")

def plot_training_results(stats: Dict, save_dir: str):
    """Génère les graphiques de l'entraînement - une image par graphe"""
    ...
    plt.tight_layout()
    save_figure('01_recompenses.png')
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    save_figure('02_taux_succes.png')
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    save_figure('03_perte_entrainement.png')
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    save_figure('06_puissance_moyenne.png')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Entraînement DDQN sur points fixes')
    parser.add_argument('--episodes', type=int, default=5000, help='Nombre d\'épisodes')
    parser.add_argument('--save-dir', type=str, default='./ddqn_fixed_points', 
                       help='Répertoire de sauvegarde')
    parser.add_argument('--resume', type=str, default=None, 
                       help='Reprendre depuis un checkpoint')
    parser.add_argument('--sampling', type=str, default='random',
                       choices=['sequential', 'random', 'curriculum'],
                       help='Mode d\'échantillonnage des distances')
    
    args = parser.parse_args()
    
    train_ddqn_fixed_points(
